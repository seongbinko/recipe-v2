...
<div class="container">
    ...
    <div class="form-block">
        <form action="sign_up" class="needs-validation col-sm-6" method="post">
            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
            <div class="form-group">
                <label for="nickname">닉네임</label>
                <c:if test="${param.nickname eq null}">
                    <input aria-describedby="nicknameHelp" class="form-control" id="nickname" name="nickname" required
                           maxlength="20"
            ...
            <div class="form-group">
                ...
                <c:if test="${param.email eq null}">
                    <input aria-describedby="emailHelp" type="email" class="form-control" id="email" name="email"
                           required placeholder="your@email.com">
                </c:if>
                <c:if test="${param.email ne null}">
                    <input aria-describedby="emailHelp" type="email" class="form-control" id="email" name="email"
                           required placeholder="your@email.com" value="${param.email}">
                </c:if>
                ...
                <c:if test="${errors['email'] ne null}">
                    <small class="form-text text-danger">Email
                        Error: <c:out value="${errors['email']}"/></small>
                </c:if>
            </div>
            ...
            <div class="form-group">
                ...
                <c:if test="${param.password eq null}">
                    <input aria-describedby="passwordHelp" class="form-control" id="password" name="password" required
                           maxlength="50"
                ...
                <c:if test="${errors['password'] ne null}">
                    <small class="form-text text-danger">Password
                        Error: <c:out value="${errors['password']}"/></small>
                </c:if>
            </div>
